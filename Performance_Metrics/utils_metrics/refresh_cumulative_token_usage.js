// Performance_Metrics/utils_metrics/refresh_cumulative_token_usage.js
...
/**
 * Refreshes token usage logs for a given agentId
 * Updates IndexedDB and re-renders ONLY token usage chart
 ...
 */
async function refreshTokenUsageLogs(agentId, timeframe = "lifetime") {
  try {
    if (!agentId) {
      throw new Error("agentId is required to fetch token usage logs");
    }
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch token usage logs: ${response.statusText}`);
    }
      ...
      dbRequest.onsuccess = async () => {
        ...
        getReq.onsuccess = async () => {
          ...
          putReq.onsuccess = async () => {
            console.log(`✅ usageLogs updated for agentId=${agentId}`);

            try {
              // 3. Fetch updated agent and re-render ONLY token usage chart
              const updatedAgent = await getAgentFromIndexedDB(agentId);

              if (typeof createTokenUsageChart === "function") {
                ...
                // Find which period button is active for tokenChart
                let activeBtn = document.querySelector(
                  `.time-period-buttons button.active[data-chart="tokenChart"]`
                );
                let period = activeBtn ? activeBtn.dataset.period : "days";

                // Recreate token usage chart with the same selected period
                createTokenUsageChart(updatedAgent);
                console.log(`🔄 Token usage chart refreshed with period: ${period}`);
              }

            } catch (err) {
              console.error("⚠️ Could not reload token usage chart with updated agent:", err);
            }
          ...
          putReq.onerror = () => {
            console.error("❌ Failed to update usageLogs in IndexedDB");
            resolve(false);
          };
        };
    ...
    console.error("Error in refreshTokenUsageLogs:", err);
    return false;
  }
}

// Attach refresh button listener for token usage
window.addEventListener("load", () => {
  const agentId = getAgentIdFromUrl();
  const tokenRefreshBtn = document.querySelector("#tokenUsageRefreshBtn");

  if (tokenRefreshBtn && agentId) {
    tokenRefreshBtn.addEventListener("click", async () => {
      console.log("🔄 Token usage refresh button clicked for agent:", agentId);

      // Get active time period for token chart
      const activeBtn = document.querySelector(
        `.time-period-buttons button.active[data-chart="tokenChart"]`
      );
      const timeframe = activeBtn ? mapPeriodToTimeframe(activeBtn.dataset.period) : "lifetime";

      const success = await refreshTokenUsageLogs(agentId, timeframe);

      if (success) {
        console.log("✅ Token usage logs refreshed & chart updated");
      } else {
        console.error("❌ Failed to refresh token usage logs");
      }
    });
  }
});
